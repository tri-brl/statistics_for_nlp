...
LAYERS           = [3, 12, 21]     # lower, middle, upper (model has 24 layers 0-23)
TARGET_SR        = 16_000
DEVICE           = "cuda" if torch.cuda.is_available() else "cpu"
XLSR_FRAME_RATE  = 49.0            # ~20ms hop after CNN feature extractor
...
def get_frame_indices(onset: float, offset: float, n_frames: int) -> list[int]:
    """
    Return XLS-R frame indices overlapping with [onset, offset].
    XLS-R CNN subsamples audio to ~49 frames/sec.
    """
    start_frame = max(0, int(onset * XLSR_FRAME_RATE))
    end_frame   = min(n_frames, int(np.ceil(offset * XLSR_FRAME_RATE)))
...
for wav_idx, ((spk, tg), group) in enumerate(grouped):
    ...
    try:
        ...
        # hidden_states: tuple of (1, n_frames, hidden_size)
        # index 0 = CNN feature extractor output, 1..24 = transformer layers
        ...
        for _, row in group.iterrows():
            ...
            log_rows.append({
                ...
                "elapsed":  round(elapsed / n_phonemes, 4),
                "status":   "ok",
            })
...
# ── save ──────────────────────────────────────────────────────────────────────
for layer in LAYERS:
    out_path = OUTPUT_DIR / f"features_xlsr_L{layer}.npz"
...
log_df.to_csv(OUTPUT_DIR / "features_xlsr_log.csv", index=False)
